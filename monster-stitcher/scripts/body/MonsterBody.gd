...
# Internal
# ══════════════════════════════════════════════════════════════════════════════
func find_all_child_parts(parent_part: BodyPartData, recursive: bool = true):
	var ret: Array[BodyPartData] = []

	for part in parts:
		if part.socket_node.parent_socket_id == parent_part.socket_id:
			ret.append(part)

	if recursive:
		for part in ret:
			ret.append_array(find_all_child_parts(part, recursive))
...
func _on_part_destroyed(destroyed_part: BodyPartData) -> void:
	...
	var child_parts = find_all_child_parts(destroyed_part, true)
...
# Visuals
# ══════════════════════════════════════════════════════════════════════════════
func build_monster_scene() -> Node2D:
	...
	for socket in sockets:
		...
		if socket.parent_socket_id != -1:
			for socket_two in sockets:
				if socket_two.socket_id == socket.parent_socket_id:
					socket.update_parent_socket(socket_two)
					break
