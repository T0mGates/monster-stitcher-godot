...
@onready var anim_player  : BattleAnimationPlayer = $BattleAnimationPlayer
...
func _ready() -> void:
	...
	player_rig.add_child(pm.build_monster_scene())
	enemy_rig.add_child(em.build_monster_scene())

	_register_animators(pm)
	_register_animators(em)

	for a in anim_player._part_animators.values():
		(a as BodyPartAnimator).play(Enums.ClipRole.IDLE)

	anim_player.connect_signals()
...
# ══════════════════════════════════════════════════════════════════════════════
func _register_animators(monster: MonsterBody) -> void:
	for part in monster.parts:
		var animator := part.socket_node.get_body_part_animator()

		if animator == null:
			continue

		anim_player.register_part_animator(part.part_id, animator)
